...
def round_transition_screen(seconds, screen, round_counter, num_surviving, num_dead, num_eaten):
    ...
    message_dead = "{num_dead} creatures died from hunger"
    message_eaten = "{num_eaten} creatures eaten"
    ...
    text_eaten, rect_eaten = sub_font.render(message_eaten.format(num_eaten=num_eaten), (0,0,0))
    ...
    screen.blit(text_eaten, (int((SCREEN_WIDTH - rect_eaten[2])/2), int((SCREEN_HEIGHT - rect_eaten[3])/2) + 114))
